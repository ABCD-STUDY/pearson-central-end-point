...
/**
 *
 * Data store end-point for Pearson's Q-interactive Central application.
 *
 ...
 *
 * Login:
 *   curl -F "action=test" https://abcd-report.ucsd.edu/applications/pearson-central-end-point/d/sA.php
 *   Result: Error HTML "Unauthorized"
 *
 *   curl --user <user name> -F "action=test" https://abcd-report.ucsd.edu/applications/pearson-central-end-point/d/sA/r.php
 *   Result: asks for password for the given user, responds with  { "message": "ok" }
 *
 * Store files:
 *   curl --user <user name> -F "action=store" https://abcd-report.ucsd.edu/applications/pearson-central-end-point/d/sA/r.php
 *   Result: Error json message: {"message":"Error: no files attached to upload"}
 *
 *   echo "1,2,3,4" > test.csv
 *   curl --user <user name> -F "action=store" -F "upload=@test.csv" https://abcd-report.ucsd.edu/applications/pearson-central-end-point/d/sA/r.php
 *   Result: A single file is stored, json object with error=0 returned
 *  
 *   echo "1,2,3,4,5" > test2.csv
 *   curl --user <user name> -F "action=store" -F "upload[]=@test.csv" -F "upload[]=@test2.csv" https://abcd-report.ucsd.edu/applications/pearson-central-end-point/d/sA/r.php
...
if ($action == 'test') {
   ...
   if ($_FILES['upload']) {
     $uploads_dir = '/var/www/html/applications/pearson-central-end-point/d/'.$site;
